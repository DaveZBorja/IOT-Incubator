...
#define DHTPIN 0 // Pin where the DHT11 sensor is connected
...
#define RELAY2 12 // Pin for relay controlling the humidity system
...
void setup() {
  ...
  digitalWrite(RELAY1, LOW); // Initialize relays as off
  digitalWrite(RELAY2, LOW);

  WiFi.softAP("Incubator", "password");
  ...
  // Initialize EEPROM to store and read threshold values
  EEPROM.begin(512);
...
void handleAutomaticControl() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (currentHumidity < humidityThreshold) {
      digitalWrite(RELAY2, HIGH); // Turn humidity system ON
      Serial.println("Humidity System ON");
    } else {
      digitalWrite(RELAY2, LOW); // Turn humidity system OFF
